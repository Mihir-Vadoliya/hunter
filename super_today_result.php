                              ...
                              <button class="nav-link active bg-transparent" id="allid-tab" data-toggle="tab" data-target="#allid" type="button" role="tab" aria-controls="allid" aria-selected="true"><strong class="text-light">
                                    <h4 class="m-0 font-weight-bold text-dark">
                                       <font size="+2">Today Result:
                                          <?php
                                          $todaydatetodaysuper =  date('Y-m-d');
                                          $querytodaysuper = "SELECT * FROM tickets where UserID in (SELECT user_id FROM details where agent_user in (Select p_id from userinfo where super_id  = '$admin_name'))";
                                          $resulttodaysuper = $con->query($querytodaysuper);
                                          $totalsuperseller = mysqli_num_rows($resulttodaysuper);
                                          if ($resulttodaysuper->num_rows > 0) {
                                             $i = 0;
                                             while ($rowpantodaysuper = $resulttodaysuper->fetch_assoc()) {
                                                $regdatetodaysuper =  $rowpantodaysuper['PNR_Time'];
                                                $registrationdatetodaysuper = date("Y-m-d", strtotime($regdatetodaysuper));
                                                if ($todaydatetodaysuper == $registrationdatetodaysuper) {
                                                   $i++;
                                                }
                                             }
                                             echo $i;
                                             ...
                                             <div class="table-responsive">
                                                <table data-page-length='25' class="table table-hover table-bordered border-primary table-md display" id="" width="100%" cellspacing="0">
                                                   ...
                                                   <tbody>
                                                      ...
                                                      $query = "SELECT * FROM tickets where UserID in (SELECT user_id FROM details where agent_user in (Select p_id from userinfo where super_id  = '$admin_name'))";
                              ...
                              <div class="tab-pane fade" id="activeid" role="tabpanel" aria-labelledby="activeid-tab">
                                 <div class="tile">
                                    <div class="tile-body">
                                       <div class="card shadow mb-4">
                                          <div class="card-header py-1" style="background-image:linear-gradient(180deg,#009688 10%,#009688 100%)">
                                             <h4 class="text-light"><strong>Total Result: <?php echo $totalsuperseller; ?></strong></h4>
                                          </div>
                                          <div class="card-body">
                                             <div class="table-responsive">
                                                <table data-page-length='25' class="table table-hover table-bordered border-primary table-md display" id="" width="100%" cellspacing="0">
                                                   ...
                                                   <tbody>
                                                      ...
                                                      $todaydate =  date('Y-m-d');
                                                      $querytotaloutcomes = "SELECT * FROM tickets where UserID in (SELECT user_id FROM details where agent_user in (Select p_id from userinfo where super_id  = '$admin_name'))";
